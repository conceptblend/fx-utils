...
/**
 * Repeats a function some number of times.
 * @param {Number} n - Number of times to repeat
 * @param {Function} fn - The function to call 'n' times
 * @returns {void}
 * @example
 * // outputs 0,1,2,3
...
export function repeat(n, fn) {
  if (typeof fn !== "function") throw new Error(`repeat(): Expected 'fn' to be a Function but got type '${typeof fn}'`);
  if (n <= 0) throw new Error(`repeat(): Expected 'n' to be greater than 0 but got '${n}'`);
  for (let i = 0; i < n; i++) {
    fn.call(this, i);
  }
}
